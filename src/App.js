...
const P = {
  bg: "#fdf6f0", panel: "#ffffff", border: "#e8d5c4",
  accent: "#b76e79", accent2: "#d4956a", accent3: "#8faf8f", accent4: "#9b7fa6",
  text: "#3d2b2b", muted: "#a89080", gold: "#c9a96e", blush: "#f2d4d4",
};
...
const GROUPS=[
  ...
  {label:"Graph",color:P.accent4,items:[{id:"BFS",label:"BFS",full:"Breadth-First Search",type:"graph"},{id:"DFS",label:"DFS",full:"Depth-First Search",type:"graph"}]},
];
...
function Bars({step}){
  ...
  return <div style={{display:"flex",alignItems:"flex-end",gap:5,flexWrap:"wrap",minHeight:160,padding:"10px 0"}}>
    {a.map((v,i)=>{
      ...
      else if(step.active?.includes(i))c=P.accent;
      else if(step.sorted instanceof Set&&step.sorted.has(i))c=P.accent3;
      else if(step.merging?.includes(i))c=`${P.accent}99`;
...
function Table2D({step,maxC=13}){
  ...
  return <div style={{overflowX:"auto"}}><table style={{borderCollapse:"collapse",fontFamily:"inherit",fontSize:12}}>
    <thead><tr><th style={{padding:"4px 8px",color:P.muted,fontWeight:500}}>i\j</th>
      {Array.from({length:cols},(_,j)=><th key={j} style={{padding:"4px 8px",color:P.accent,fontWeight:700}}>{step.s2?(["-",...step.s2])[j]??j:j}</th>)}
    </tr></thead>
    <tbody>{step.dp.map((row,i)=><tr key={i}>
      <td style={{padding:"3px 8px",color:P.accent,fontWeight:700}}>{step.s1?(["-",...step.s1])[i]??i:i}</td>
      {row.slice(0,cols).map((v,j)=>{const isA=step.cell&&step.cell[0]===i&&step.cell[1]===j;return <td key={j} style={{padding:"5px 10px",textAlign:"center",fontWeight:isA?700:400,color:isA?"#fff":v>0?P.text:P.muted,background:isA?P.accent:v>0?P.blush:"transparent",border:`1px solid ${isA?P.accent:P.border}`,transition:"all 0.2s",minWidth:30,borderRadius:4}}>{v}</td>;})}
    </tr>)}</tbody>
  </table></div>;
}
...
function Controls({idx,setIdx,playing,setPlaying,speed,setSpeed,steps,logRef}){
  ...
  return <div>
    <div style={{display:"flex",gap:6,alignItems:"center",flexWrap:"wrap",marginBottom:12}}>
      ...
      <button onClick={()=>setPlaying(p=>!p)} style={{background:`linear-gradient(135deg,${P.accent},${P.accent2})`,color:"#fff",fontWeight:700,padding:"8px 22px",fontSize:14,borderRadius:20,boxShadow:`0 4px 15px ${P.accent}40`}}>{playing?"⏸ Pause":"▶ Play"}</button>
      ...
      <span style={{color:P.muted,fontSize:13,marginLeft:4,fontWeight:500}}>{idx+1} / {steps.length}</span>
      <div style={{display:"flex",gap:4,marginLeft:"auto"}}>
        {[["0.5×",1200],["1×",700],["2×",350],["4×",120]].map(([l,v])=>(
          <button key={l} onClick={()=>setSpeed(v)} style={{padding:"4px 10px",fontSize:12,borderRadius:12,background:speed===v?P.blush:"transparent",color:speed===v?P.accent:P.muted,border:`1px solid ${speed===v?P.accent:P.border}`,fontWeight:speed===v?700:400}}>{l}</button>
        ))}
      </div>
    </div>
    <div style={{height:5,background:P.border,borderRadius:4,overflow:"hidden",marginBottom:12}}>
    ...
    <div style={{background:P.blush,border:`1px solid ${P.border}`,borderRadius:12,padding:"12px 16px",fontSize:15,color:step?(phaseColor[step.phase]||P.text):P.muted,lineHeight:1.7,minHeight:50,fontWeight:500}}>
      {step?.msg||"—"}
    </div>
    <div ref={logRef} style={{marginTop:8,background:"#fdf0f0",border:`1px solid ${P.border}`,borderRadius:12,padding:10,height:95,overflowY:"auto",fontSize:12}}>
...
export default function App(){
  ...
  const[steps,setSteps]=useState([]);const[result,setResult]=useState(null);
  const pb=usePlayback(steps);
  const ca=GROUPS.flatMap(g=>g.items).find(a=>a.id===algoId);
  const runB=useCallback(()=>{const{steps:s,result:r}=runBuiltin(algoId,inp);setSteps(s);setResult(r);},[algoId,inp]);
  useEffect(()=>{setSteps([]);setResult(null);},[algoId]);
  const bs=steps[pb.idx];

  const Card=({children,style={}})=><div style={{background:P.panel,border:`1px solid ${P.border}`,borderRadius:16,padding:22,boxShadow:"0 2px 20px rgba(183,110,121,0.07)",...style}}>{children}</div>;
  const Label=({children,color=P.accent})=><div style={{color,fontSize:11,fontWeight:700,letterSpacing:2,textTransform:"uppercase",marginBottom:14,display:"flex",alignItems:"center",gap:6}}><div style={{width:6,height:6,borderRadius:"50%",background:color,boxShadow:`0 0 6px ${color}`}}/>{children}</div>;

  return <div style={{minHeight:"100vh",background:`linear-gradient(135deg,#fdf6f0 0%,#fde8e8 50%,#f5e8f5 100%)`,color:P.text,fontFamily:"'Inter','Helvetica Neue',sans-serif",padding:"24px 16px"}}>
    <style>{`@import url('https://fonts.googleapis.com/css2?family=Inter:wght@400;500;600;700;800;900&display=swap');*{box-sizing:border-box}::-webkit-scrollbar{width:5px;height:5px}::-webkit-scrollbar-thumb{background:${P.border};border-radius:10px}input{background:#fff;border:2px solid ${P.border};color:${P.text};padding:10px 14px;border-radius:12px;font-family:inherit;font-size:14px;outline:none;width:100%;transition:border .2s}input:focus{border-color:${P.accent};box-shadow:0 0 0 3px ${P.accent}20}button{cursor:pointer;border:none;font-family:inherit;transition:all .18s}button:disabled{opacity:.35;cursor:not-allowed}button:hover:not(:disabled){transform:translateY(-1px)}@keyframes fadeUp{from{opacity:0;transform:translateY(10px)}to{opacity:1;transform:translateY(0)}}`}</style>
    <div style={{maxWidth:900,margin:"0 auto"}}>
      <div style={{textAlign:"center",marginBottom:30}}>
        <div style={{fontSize:12,color:P.gold,fontWeight:700,letterSpacing:5,textTransform:"uppercase",marginBottom:8}}>✦ Beautiful Learning ✦</div>
        <h1 style={{fontSize:48,fontWeight:900,margin:0,background:`linear-gradient(135deg,${P.accent},${P.gold})`,WebkitBackgroundClip:"text",WebkitTextFillColor:"transparent",letterSpacing:-1}}>AlgoViz</h1>
        <p style={{color:P.muted,fontSize:16,marginTop:8,fontWeight:500}}>Step-by-step algorithm visualization, beautifully designed</p>
      </div>
      ...
      <div style={{display:"flex",flexDirection:"column",gap:16,animation:"fadeUp .3s ease"}}>
        ...
        <Card>
          ...
          {(ca?.type==="array"||ca?.type==="bsearch")&&<div style={{marginBottom:12}}><label style={{color:P.muted,fontSize:13,display:"block",marginBottom:6,fontWeight:600}}>Array values</label><input value={inp.arr} onChange={e=>setInp(p=>({...p,arr:e.target.value}))}/></div>}
          {ca?.type==="bsearch"&&<div style={{marginBottom:12}}><label style={{color:P.muted,fontSize:13,display:"block",marginBottom:6,fontWeight:600}}>Target value</label><input value={inp.target} onChange={e=>setInp(p=>({...p,target:e.target.value}))}/></div>}
          {ca?.type==="strings"&&<div style={{display:"flex",gap:12,marginBottom:12,flexWrap:"wrap"}}><div style={{flex:1,minWidth:120}}><label style={{color:P.muted,fontSize:13,display:"block",marginBottom:6,fontWeight:600}}>String 1</label><input value={inp.s1} onChange={e=>setInp(p=>({...p,s1:e.target.value}))}/></div><div style={{flex:1,minWidth:120}}><label style={{color:P.muted,fontSize:13,display:"block",marginBottom:6,fontWeight:600}}>String 2</label><input value={inp.s2} onChange={e=>setInp(p=>({...p,s2:e.target.value}))}/></div></div>}
          {ca?.type==="knapsack"&&<div style={{display:"flex",gap:12,marginBottom:12,flexWrap:"wrap"}}><div style={{flex:2,minWidth:100}}><label style={{color:P.muted,fontSize:13,display:"block",marginBottom:6,fontWeight:600}}>Weights</label><input value={inp.w} onChange={e=>setInp(p=>({...p,w:e.target.value}))}/></div><div style={{flex:2,minWidth:100}}><label style={{color:P.muted,fontSize:13,display:"block",marginBottom:6,fontWeight:600}}>Values</label><input value={inp.v} onChange={e=>setInp(p=>({...p,v:e.target.value}))}/></div><div style={{flex:1,minWidth:70}}><label style={{color:P.muted,fontSize:13,display:"block",marginBottom:6,fontWeight:600}}>Capacity</label><input value={inp.cap} onChange={e=>setInp(p=>({...p,cap:e.target.value}))}/></div></div>}
          {ca?.type==="graph"&&<p style={{color:P.muted,fontSize:14,margin:"0 0 12px",fontWeight:500}}>7-node graph A through G — no input needed! 🌸</p>}
          <button onClick={runB} style={{background:`linear-gradient(135deg,${P.accent},${P.accent2})`,color:"#fff",fontWeight:700,padding:"11px 26px",fontSize:15,borderRadius:20,boxShadow:`0 4px 20px ${P.accent}40`,letterSpacing:.5}}>▶ Run Algorithm</button>
        </Card>

        <Card>
          <Label>Visualization</Label>
          {!bs&&<div style={{color:P.muted,textAlign:"center",padding:40,fontSize:15,fontWeight:500}}>Press Run Algorithm to start ✨</div>}
          {bs&&(ca?.type==="array"||ca?.type==="bsearch")&&<Bars step={bs}/>}
          {bs&&algoId==="LIS"&&bs.dp&&<><div style={{color:P.muted,fontSize:12,marginTop:14,fontWeight:600,letterSpacing:1}}>DP ARRAY:</div><div style={{display:"flex",gap:6,flexWrap:"wrap",marginTop:8}}>{bs.dp.map((v,i)=>{const isA=bs.active?.includes(i),isC=bs.comparing?.includes(i);return <div key={i} style={{textAlign:"center"}}><div style={{width:38,height:38,display:"flex",alignItems:"center",justifyContent:"center",background:isA?P.blush:isC?"#fde8d8":"#fdf6f0",border:`2px solid ${isA?P.accent:isC?P.accent2:P.border}`,color:isA?P.accent:P.text,fontWeight:700,fontSize:14,borderRadius:8,transition:"all 0.2s"}}>{v}</div><div style={{color:P.muted,fontSize:10,marginTop:3}}>dp[{i}]</div></div>;})}</div></>}
        ...
        {steps.length>0&&<Card><Label>Playback Controls</Label><Controls {...pb} steps={steps}/></Card>}

        {result!==null&&pb.idx===steps.length-1&&steps.length>0&&(
          <div style={{background:`linear-gradient(135deg,${P.blush},#f5e8f5)`,border:`1.5px solid ${P.accent}`,borderRadius:16,padding:"22px 28px",textAlign:"center",boxShadow:`0 8px 30px ${P.accent}20`}}>
            <div style={{color:P.muted,fontSize:11,fontWeight:700,letterSpacing:3,textTransform:"uppercase",marginBottom:10}}>✦ Result</div>
            <div style={{fontSize:Array.isArray(result)?18:36,fontWeight:900,background:`linear-gradient(135deg,${P.accent},${P.gold})`,WebkitBackgroundClip:"text",WebkitTextFillColor:"transparent",wordBreak:"break-all"}}>{Array.isArray(result)?`[ ${result.join(", ")} ]`:String(result)}</div>
            <div style={{color:P.muted,fontSize:14,marginTop:8,fontWeight:500}}>{ca?.full}</div>
          </div>
        )}

        <div style={{display:"flex",gap:16,flexWrap:"wrap",justifyContent:"center",paddingTop:8,paddingBottom:20}}>
          {[[P.accent,"Active"],[P.accent2,"Comparing"],[P.accent3,"Sorted"],[P.accent4,"Swap"],[P.gold,"Pivot"],[P.muted,"Default"]].map(([c,l])=>(
            <div key={l} style={{display:"flex",alignItems:"center",gap:6}}><div style={{width:9,height:9,background:c,borderRadius:3,boxShadow:`0 0 6px ${c}70`}}/><span style={{color:P.muted,fontSize:13,fontWeight:500}}>{l}</span></div>
          ))}
        </div>
      </div>
    </div>
